# exam/semester_exam.py
from . import exam_bp, _execute, _to_date
from flask import request, jsonify

@exam_bp.route('/semester/create', methods=['POST'])
def create_semester_exam():
    """
    POST JSON:
    {
    ...
    q = """INSERT INTO exam_schedule (exam_type, exam_date, session, course_id)
           VALUES (%s, %s, %s, %s)"""
    res = _execute(q, ("Semester", exam_date, session, course_id))
    if isinstance(res, tuple) and res[1] == 500:
        return jsonify(res[0]), 500
    return jsonify({"ok": True, "message":"Semester exam created"}), 201


@exam_bp.route('/semester/list', methods=['GET'])
def list_semester_exams():
    q = "SELECT * FROM exam_schedule WHERE exam_type = 'Semester' ORDER BY exam_date, session"
